<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Laporan Inventaris - Inventory Masjid</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <style>
        @media print {
            body { 
                print-color-adjust: exact; 
                -webkit-print-color-adjust: exact;
            }
            .no-print { display: none !important; }
        }
    </style>
</head>
<body class="bg-white p-8">
    <div class="no-print mb-4">
        <button onclick="window.print()" class="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-lg font-semibold">
            🖨️ Cetak
        </button>
        <a href="<?php echo e(route('reports.index')); ?>" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-6 py-2 rounded-lg font-semibold ml-2">
            ← Kembali
        </a>
    </div>

    <div class="text-center mb-8">
        <h1 class="text-2xl font-bold">🕌 LAPORAN INVENTARIS MASJID</h1>
        <p class="text-gray-600 mt-2">Tanggal: <?php echo e(now()->format('d/m/Y')); ?></p>
        <p class="text-gray-600">Kategori: <?php echo e($categoryName); ?> | Lokasi: <?php echo e($locationName); ?></p>
    </div>

    <table class="min-w-full border border-gray-300">
        <thead>
            <tr class="bg-gray-100">
                <th class="border border-gray-300 px-4 py-2 text-left text-sm font-semibold">No</th>
                <th class="border border-gray-300 px-4 py-2 text-left text-sm font-semibold">Nama Barang</th>
                <th class="border border-gray-300 px-4 py-2 text-left text-sm font-semibold">Kategori</th>
                <th class="border border-gray-300 px-4 py-2 text-left text-sm font-semibold">Lokasi</th>
                <th class="border border-gray-300 px-4 py-2 text-left text-sm font-semibold">Jumlah</th>
                <th class="border border-gray-300 px-4 py-2 text-left text-sm font-semibold">Kondisi</th>
                <th class="border border-gray-300 px-4 py-2 text-left text-sm font-semibold">Catatan</th>
            </tr>
        </thead>
        <tbody>
            <?php $__empty_1 = true; $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <tr>
                <td class="border border-gray-300 px-4 py-2 text-sm"><?php echo e($index + 1); ?></td>
                <td class="border border-gray-300 px-4 py-2 text-sm font-medium"><?php echo e($item->name); ?></td>
                <td class="border border-gray-300 px-4 py-2 text-sm"><?php echo e($item->category->name); ?></td>
                <td class="border border-gray-300 px-4 py-2 text-sm"><?php echo e($item->location->name); ?></td>
                <td class="border border-gray-300 px-4 py-2 text-sm"><?php echo e($item->quantity); ?> <?php echo e($item->unit); ?></td>
                <td class="border border-gray-300 px-4 py-2 text-sm">
                    <?php if($item->condition == 'baik'): ?>
                        Baik
                    <?php elseif($item->condition == 'perlu_perbaikan'): ?>
                        Perlu Perbaikan
                    <?php else: ?>
                        Rusak
                    <?php endif; ?>
                </td>
                <td class="border border-gray-300 px-4 py-2 text-sm"><?php echo e($item->note ?? '-'); ?></td>
            </tr>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
            <tr>
                <td colspan="7" class="border border-gray-300 px-4 py-4 text-center text-gray-500">Belum ada barang.</td>
            </tr>
            <?php endif; ?>
        </tbody>
    </table>

    <div class="mt-8 text-sm text-gray-600">
        <p>Total Barang: <?php echo e($items->count()); ?></p>
        <p>Kondisi Baik: <?php echo e($items->where('condition', 'baik')->count()); ?></p>
        <p>Perlu Perbaikan: <?php echo e($items->where('condition', 'perlu_perbaikan')->count()); ?></p>
        <p>Rusak: <?php echo e($items->where('condition', 'rusak')->count()); ?></p>
    </div>

    <div class="mt-12 flex justify-end">
        <div class="text-center">
            <p class="mb-16">Mengetahui,</p>
            <p class="border-t border-gray-400 pt-2">Pengurus Masjid</p>
        </div>
    </div>
</body>
</html>
<?php /**PATH /var/www/resources/views/reports/print.blade.php ENDPATH**/ ?>